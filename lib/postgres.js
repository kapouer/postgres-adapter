...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    schema.constructor.registerType(schema.adapter.GeoJSON);
...
PG.prototype.GeoJSON = function GeoJSON(s) { return s; };
...
PG.prototype.getColumns = function(model) {
    var props = this._models[model].properties;
    var self = this;
    var mkeys = Object.keys(props).map(function(colname, i) {
        var prop = props[colname];
        var escol = self.escapeName(colname);
        if (props[colname].type.name == "GeoJSON") {
            return 'ST_AsGeoJSON(' + escol + ') AS ' + escol;
        } else {
            return escol;
        }
    });
...
function addPropertyToActual(model, propName){
    ...
    var sqlCommand = 'ADD COLUMN ' + this.propertySettingsSQL(model, propName);
    return sqlCommand;
};
...
function datatype(p) {
    switch (p.type.name) {
        ...
        return 'boolean';
        case 'GeoJSON':
        return 'geometry(Geometry, 4326)'; // works with postgis >= 2.0.0
    }
};
